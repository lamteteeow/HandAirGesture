...
with mp_hands.Hands(
  ...
  while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
      for hand_landmarks in results.multi_hand_landmarks:
        # Calculate distance between index finger and thumb for thresholding
        ...
        dis = ((index_tip[0] - thumb_tip[0])**2 + (index_tip[1] - thumb_tip[1])**2) ** 0.5
        print(dis)
        # If distance is less than 0.09, left-doubleclick
        if dis < 0.05:
          pdi.doubleClick()
        # Move cursor relative to wrist position
        pdi.moveTo(int((1- wrist[0]) * 1920), int(wrist[1] * 1080))    
        mp_drawing.draw_landmarks(
            image,
            hand_landmarks,
            mp_hands.HAND_CONNECTIONS,
